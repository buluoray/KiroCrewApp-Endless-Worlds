...
| [systems.md](systems.md) | The game-system plugin contract: world-declared mechanics, the accrual/resource/decay/unlock transitions, prior-state ownership, gain matching, fail-soft application, and the synchronized workflow for declaring a mechanic or adding an engine kind. |
...
| [scenes-and-backdrop.md](scenes-and-backdrop.md) | The scene compiler/ledger split and the backdrop validation funnel: a closed element-kind set, backend-computed geometry, whole-spec validation, a CSP-first document loaded through a sandboxed iframe `src` with no same-origin grant, the answer nonce that is never narrator-facing, and the inert-`<img>` "never run it at all" backdrop model. |
