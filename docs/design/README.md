...
| [world-as-data.md](world-as-data.md) | Data-izing the world: `lore` gains structure (name/summary/category/relations/reveal + a user-facing setting view), backend-managed `systems` (xp/resource/decay/unlock), `roles`, and `opening.handToAgent`. The world becomes open-vocabulary data the backend manages, the narrator declares events, and the user browses the setting. No new concepts (reuses `lore`/`panels`/`opening`). Phased P1–P4. |
| [fate-check-design.md](fate-check-design.md) | Whether and how to give a world real odds: an `endless_roll` fate check the narrator invokes freely, a world may optionally declare, and no one may re-roll — plus the anti-frustration constraints (named generous odds, fail-forward outcomes, `fateful`-level rarity). Design only; nothing built. Its one landed prerequisite is the gain-identity invariant in [../modules/systems.md](../modules/systems.md). |
