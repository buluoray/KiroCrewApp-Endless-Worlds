...
入口放在人生页的次级操作区，与“看看这一刻的自己”和历史回看同级，不放在每回合行动按钮旁。
...
视觉实现前必须制作三套真实 mockup：

1. **时间星座，推荐**：时间轴是骨架，人物与地点围绕关键事件分簇；最适合读人生因果。
2. **关系轨道**：当前角色居中，人物、地点和事件按关系距离分层；适合关系探索，但时间感较弱。
3. **纪念地图**：纪念卡是主节点，其余事实从卡片向外展开；最亲切，但不适合浏览未收藏事实。

实现应以选中的 mockup 为视觉规范，不直接采用通用 force-directed graph 默认效果。
...
- 新建 `web/src/memory.tsx`：人生星图与节点详情。
...
完成标准：收藏与删除纪念不会改变事实图或 Narrator 状态。
...
| 图 UI 变成工程工具 | 默认隐藏、疏图呈现、采用叙事化时间星座 |
...
进入实现前只剩视觉层需要用 mockup 决定：

1. 人生星图采用时间星座、关系轨道还是纪念地图。
2. “往事回响”在正文中的展开密度。
3. 回响故事卡的版式、主题变量和小关系图位置。
4. 手机宽度下星图是横向画布、分层列表还是两者切换。

推荐先制作三个自包含 HTML mockup，使用现有 `styles.css` 变量和真实中英文长文本，再由选中的 mockup 成为实现视觉规范。
